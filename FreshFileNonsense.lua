...
for seed=MinSeed,MaxSeed do
  ...
  for uses=MinC1Uses,MaxC1Uses  do
    ...
    local result = {
      C1_Seed = seed,
      C2_Seed = prediction.Seed,
      C2_DoorRewards = ExitRewards(prediction.NextExitRewards),
      C3_RoomName = prediction.NextExitRewards[1].RoomName
    }
    local c3door = {
      Room = DeepCopyTable(prediction.NextExitRewards[1].Room)
    }
    NextSeeds[1] = prediction.Seed
    RandomSynchronize(6) -- uses at end of athena room
    CurrentRun = C2Run
    local prediction = PredictLoot(c3door)
    result.C3_Seed = prediction.Seed
    result.C3_DoorRewards = ExitRewards(prediction.NextExitRewards)
    result.C3_Waves = EncounterWaves(prediction.Encounter)
    record(result)
  end
end
